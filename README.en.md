...
### New: AI Video Orchestrator

Type a film description (30–180 seconds) → the system automatically splits it into segments, generates visually consistent prompts, and dispatches video generation to your chosen web AI service. Finished segments are assembled into a single film via ffmpeg.

**What's new:**
- **Video AI tab** with a 4-step wizard (prompt → settings → plan preview → generation)
- **Two-pass PromptProcessor** — one prompt → coherent plan with color palette, mood, camera style, and frame chaining between segments
- **5 supported web services** (via Playwright — uses your subscriptions, no per-call API fees): Pixverse, Hailuo/MiniMax, Gemini Veo, Kling AI, Runway ML
- **In-UI session bootstrap** — click "Login" next to a service → browser opens in an iframe (noVNC) → log in once → session saved automatically
- **Automatic failover** between services, checkpointing, safe restart
- **`local_ffmpeg` driver** — always available, no login required, for testing the full pipeline
...
The goal was voice cloning for presentation narration in Polish and English, without hallucinations or artifacts.
...
| **Fish Speech** | ✅ | ✅ | LLM-based architecture — generating 60s of audio takes 2–4 min |
...
**AI Video Orchestrator — generate long-form videos via web AI services:**
- New **Video AI** tab — 4-step wizard: prompt → settings → plan preview → generation
- **Two-pass PromptProcessor** (DirectorPass + StoryboardPass via local LLM): one user prompt → plan with `anchor` (style, color palette, mood, camera) + N segments with visual consistency and frame chaining (end of segment N = start of segment N+1)
- **5 web services** via Playwright: Pixverse (8s/seg, Google SSO), Hailuo/MiniMax (6s/seg, top quality), Gemini Veo (8s/seg), Kling AI (10s/seg, 66 credits/day free), Runway ML (10s/seg)
- **In-UI session bootstrap** — click "Login" next to a service → browser opens in an iframe (noVNC) → log in once → session saved automatically. No VNC or SSH required.
- **Automatic failover**: if one service fails, the orchestrator moves to the next in the queue
- **FFmpeg pipeline**: segment normalization (1920×1080, 30fps) → concatenation (hard-cut or crossfade) → audio mux
- **`local_ffmpeg` driver**: always available placeholder — useful for testing the full pipeline without any login
- Orchestrator with checkpointing — safe service restart during generation
